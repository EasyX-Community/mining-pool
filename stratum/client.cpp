...
bool client_validate_user_address(YAAMP_CLIENT *client)
{
	...
	YAAMP_COIND *coind = (YAAMP_COIND *)object_find(&g_list_coind, client->coin_id);
	if (!coind) {
		clientlog(client, "unable to find the wallet for coin_id %d...", client->coin_id);
		return false;
	} else {
	...
	if (isvalid) {
		...
		clientlog(client, "unable to verify %s address for user coin_id %d...", coind->symbol, client->coin_id);
	}
	return isvalid;
}
...
bool client_update_block(YAAMP_CLIENT *client, json_value *json_params)
{
	...
	int coin_id = json_params->u.array.values[1]->u.integer;
	if(!coin_id) return false;
	YAAMP_COIND *coind = (YAAMP_COIND *)object_find(&g_list_coind, coin_id, true);
	...
	if (!strcmp("DCR", coind->rpc_encoding)) {
		usleep(300*YAAMP_MS);
	}
...
int client_workers_count(YAAMP_CLIENT *client)
{
	...
	for(CLI li = g_list_client.first; li; li = li->next)
	{
		...
		if (cli->user_id == client->user_id) count++;
	}
	g_list_client.Leave();
	...
	for(CLI li = g_list_client.first; li; li = li->next)
	{
		...
		if (client->user_id) {
			if(cli->user_id == client->user_id) {
				client->coin_id = cli->coin_id;
				break;
			}
		} else if (strcmp(cli->username, client->username) == 0) {
			client->coin_id = cli->coin_id;
			client->user_id = cli->user_id;
			break;
		}
	}
	g_list_client.Leave();

	return (client->coin_id > 0 && client->user_id > 0);
}
