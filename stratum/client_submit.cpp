	...
	for(int i=0; i<templ->auxs_size; i++)
	{
		...
		if(hash_int <= coin_target_aux)
		{
			...
			if(b)
			{
				...
				block_add(client->user_id, client->worker_id, coind_aux->id, coind_aux->height, target_to_diff(coin_target_aux),
	...
	if(hash_int <= coin_target)
	{
		...
		if(b)
		{
			debuglog("*** ACCEPTED %s %d (diff %g) by %s (id: %d)\n", coind->name, templ->height,
				target_to_diff(hash_int), client->sock->ip, client->user_id);
			...
			if(coind->usegetwork && !strcmp("DCR", coind->rpc_encoding)) {
				// no merkle stuff
				strcpy(hash1, submitvalues->hash_hex);
			}

			block_add(client->user_id, client->worker_id, coind->id, templ->height,
...
bool client_submit(YAAMP_CLIENT *client, json_value *json_params)
{
	...
	bool is_decred = job->coind && !strcmp("DCR", job->coind->rpc_encoding);
