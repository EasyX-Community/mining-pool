...
void db_update_coinds(YAAMP_DB *db)
{
	...
	db_query(db, "SELECT id, name, rpc_host, rpc_port, rpc_user, rpc_password, rpc_encoding, master_wallet, reward, price, "
		"hassubmitblock, tx_message, enable, auto_ready, algo, pool_ttf, charity_address, charity_amount, charity_percent, "
		"reward_mul, symbol, auxpow, actual_ttf, network_ttf, use_memory_pool, has_master_nodes, algo, symbol2, "
		"rpc_curl, rpc_ssl, rpc_cert, account, multialgos, max_miners, max_shares, usesegwit "
	...
	while((row = mysql_fetch_row(result)) != NULL)
	{
		...
		if(row[6]) strcpy(coind->rpc_encoding, row[6]);
		...
		if(row[11]) coind->tx_message = atoi(row[11]);
		...
		if(row[24]) coind->use_memory_pool = atoi(row[24]);
		if(row[25]) coind->has_master_nodes = atoi(row[25]);
		...
		// force the right rpc_encoding for DCR
		if(!strcmp(coind->symbol, "DCR") && strcmp(coind->rpc_encoding, "DCR"))
			strcpy(coind->rpc_encoding, "DCR");

		// old dash masternodes coins..
		if(coind->has_master_nodes) {
...
void db_update_remotes(YAAMP_DB *db)
{
	...
	db_query(db, "select id, speed/1000000, host, port, username, password, time, price, renter_id from jobs where active and ready and algo='%s' order by time", g_stratum_algo);
...
//			continue;
		...
		int renter_id = row[8]? atoi(row[8]): 0;
		if(renter_id && !remote->renter)
			remote->renter = (YAAMP_RENTER *)object_find(&g_list_renter, renter_id);
...
//			db_query(db, "update jobs set ready=false, active=false where renter_id=%d", remote->renter->id);
//
...
void db_store_stats(YAAMP_DB *db, YAAMP_CLIENT *client, json_value *stats)
{
	...
	json_str_safe(stats, "vendor_id", svid);
	...
	db_query(db, "INSERT INTO benchmarks("
		"time, algo, type, device, arch, vendor_id, os, driver,"
		"client, khps, freq, memf, realfreq, realmemf, power, plimit, "
		"intensity, throughput, user_id )"
		...
		intensity, throughput, client->user_id);
}
