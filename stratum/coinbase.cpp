...
void coinbase_create(YAAMP_COIND *coind, YAAMP_JOB_TEMPLATE *templ, json_value *json_result)
{
	...
	else if(strcmp("DCR", coind->rpc_encoding) == 0) {
	...
	if (!coind->has_master_nodes && founder_enabled && founder) {
	...
	if(coind->charity_percent && !coind->has_master_nodes)
	{
		char script_payee[1024];
		char charity_payee[256] = { 0 };
		const char *payee = json_get_string(json_result, "payee");
	...
	else if(coind->charity_amount && !strcmp("DCR", coind->rpc_encoding))
	{
	...
	if(coind->has_master_nodes && !coind->oldmasternodes)
	{
		char script_dests[2048] = { 0 };
		char script_payee[128] = { 0 };
	...
	else if(coind->has_master_nodes && coind->oldmasternodes) /* OLD DASH style */
	{
		char charity_payee[256] = { 0 };
		const char *payee = json_get_string(json_result, "payee");
		if (payee) snprintf(charity_payee, 255, "%s", payee);
